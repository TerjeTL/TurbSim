function [b_p] = F_b_SAE(param,Nc_corr_n,map)
% [b_p] = F_b_SAE(param,Nc_corr_n,map)
%
% Computes B value given the parameters and the normalized corrected speed vector
%
...
%% Extract parameters
c1  = param(1:2);
% Compute the b_p for given N_c_corr (normalized)
b_p = map.Delta_h_max.*(c1(1).*Nc_corr_n.^2+c1(2).*Nc_corr_n.^3);
end